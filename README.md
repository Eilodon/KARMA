...
| `src/lib/keystore.ts` | `KeystoreManager` — Web3 v3 scrypt/aes-128-ctr decrypt; `encryptPrivateKeyV3` for keystore setup; `getTenant(agentId)` exposes the loaded agent's tenant for context-aware callers. |
...
`DEMO.md` documents a completed 5-transaction self-referential loop on the v3 contract (Pharos Atlantic, 2026-06-18).
...
KEYSTORE_PASSWORD=<password> pnpm demo
