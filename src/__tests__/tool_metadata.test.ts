...
describe("tool metadata", () => {
  test("system tools include MCP annotations and execution metadata", () => {
    const ping = systemTools.find(tool => tool.name === "karma_ping");
    const longTask = systemTools.find(tool => tool.name === "karma_test_long_task");
  ...
  test("test-only long task is gated by MCP_ENABLE_TEST_TOOLS", async () => {
    ...
    expect(source).toContain("karma_test_long_task");
  });
